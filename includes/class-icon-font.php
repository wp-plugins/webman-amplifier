			...
			public function admin_form() {
				//Helper variables
					...
					if ( isset( $fonticons['icons_select'] ) ) {
						$fonticons = '<ol class="inline-ol"><li><code>' . implode( '</code></li><li><code>', array_keys( $fonticons['icons_select'] ) ) . '</code></li></ol>';
						$fonticons = '<tr class="option-heading toggle"><th colspan="2">' . __( 'Click to display recent available icon classes', 'wm_domain' ) . '</th></tr></tbody><tbody><tr class="option padding-20"><td colspan="2"><div class="box yellow">' . $fonticons . '</div></td></tr></tbody><tbody>';
					} else {
						$fonticons = '';
					}
		 ...
		 */
			...
			private function zip_flatten( $file = '', $filter = array() ) {
				...
				//Create the temporary folder
					$tempdir = wm_create_folder( $this->paths['tempdir'] );
					if ( ! $tempdir ) {
