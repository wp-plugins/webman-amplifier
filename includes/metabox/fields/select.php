 ...
 */
	 ...
	 * @version     1.0
	 */
	if ( ! function_exists( 'wm_field_select' ) ) {
		function wm_field_select( $field, $page_template = null ) {
	...
	add_action( WM_METABOX_HOOK_PREFIX . 'render_' . 'select', 'wm_field_select', 10, 2 );
	 ...
	 * @version     1.0
	 */
	if ( ! function_exists( 'wm_field_select_validation' ) ) {
		function wm_field_select_validation( $new, $field, $post_id ) {
			$new = esc_attr( $new );

			return $new;
		} // /wm_field_select_validation
	} // /function_exists check

	add_action( WM_METABOX_HOOK_PREFIX . 'saving_' . 'select', 'wm_field_select_validation', 10, 3 );
