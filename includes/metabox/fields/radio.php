 ...
 */
	 ...
	 * @version     1.0
	 */
	if ( ! function_exists( 'wm_field_radio' ) ) {
		function wm_field_radio( $field, $page_template = null ) {
	...
	add_action( WM_METABOX_HOOK_PREFIX . 'render_' . 'radio', 'wm_field_radio', 10, 2 );
	 ...
	 * @version     1.0
	 */
	if ( ! function_exists( 'wm_field_radio_validation' ) ) {
		function wm_field_radio_validation( $new, $field, $post_id ) {
			$new = esc_attr( $new );

			return $new;
		} // /wm_field_radio_validation
	} // /function_exists check

	add_action( WM_METABOX_HOOK_PREFIX . 'saving_' . 'radio', 'wm_field_radio_validation', 10, 3 );
