 ...
 */
	 ...
	 * @version     1.0
	 */
	if ( ! function_exists( 'wm_field_text' ) ) {
		function wm_field_text( $field, $page_template = null ) {
	...
	add_action( WM_METABOX_HOOK_PREFIX . 'render_' . 'color',    'wm_field_text', 10, 2 );
	add_action( WM_METABOX_HOOK_PREFIX . 'render_' . 'password', 'wm_field_text', 10, 2 );
	add_action( WM_METABOX_HOOK_PREFIX . 'render_' . 'text',     'wm_field_text', 10, 2 );
	 ...
	 * @version     1.0
	 */
	if ( ! function_exists( 'wm_field_text_validation' ) ) {
		function wm_field_text_validation( $new, $field, $post_id ) {
			...
			return $new;
		} // /wm_field_text_validation
	} // /function_exists check

	add_action( WM_METABOX_HOOK_PREFIX . 'saving_' . 'color',    'wm_field_text_validation', 10, 3 );
	add_action( WM_METABOX_HOOK_PREFIX . 'saving_' . 'password', 'wm_field_text_validation', 10, 3 );
	add_action( WM_METABOX_HOOK_PREFIX . 'saving_' . 'text',     'wm_field_text_validation', 10, 3 );
	 ...
	 * @version     1.0
	 */
	if ( ! function_exists( 'wm_field_textarea' ) ) {
		function wm_field_textarea( $field, $page_template = null ) {
			//Field definition array
				$field = wp_parse_args( (array) $field, array(
						//DEFAULTS:
						//* = Required setting
	...
	add_action( WM_METABOX_HOOK_PREFIX . 'render_' . 'textarea', 'wm_field_textarea', 10, 2 );
	 ...
	 * @version     1.0
	 */
	if ( ! function_exists( 'wm_field_textarea_validation' ) ) {
		function wm_field_textarea_validation( $new, $field, $post_id ) {
			$new = wp_kses_post( $new );

			return $new;
		} // /wm_field_textarea_validation
	} // /function_exists check

	add_action( WM_METABOX_HOOK_PREFIX . 'saving_' . 'textarea', 'wm_field_textarea_validation', 10, 3 );
