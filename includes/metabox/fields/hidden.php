 ...
 */
	 ...
	 * @version     1.0
	 */
	if ( ! function_exists( 'wm_field_hidden' ) ) {
		function wm_field_hidden( $field, $page_template = null ) {
	...
	add_action( WM_METABOX_HOOK_PREFIX . 'render_' . 'hidden', 'wm_field_hidden', 10, 2 );
	 ...
	 * @version     1.0
	 */
	if ( ! function_exists( 'wm_field_hidden_validation' ) ) {
		function wm_field_hidden_validation( $new, $field, $post_id ) {
			$new = esc_attr( $new );

			return $new;
		} // /wm_field_hidden_validation
	} // /function_exists check

	add_action( WM_METABOX_HOOK_PREFIX . 'saving_' . 'hidden', 'wm_field_hidden_validation', 10, 3 );
