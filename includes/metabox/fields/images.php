 ...
 */
	 ...
	 */
	if ( ! function_exists( 'wm_field_image' ) ) {
		function wm_field_image( $field, $page_template = null ) {
	...
	add_action( WM_METABOX_HOOK_PREFIX . 'render_' . 'image', 'wm_field_image', 10, 2 );
	 ...
	 * @version     1.0
	 */
	if ( ! function_exists( 'wm_field_image_validation' ) ) {
		function wm_field_image_validation( $new, $field, $post_id ) {
			...
			return $new;
		} // /wm_field_image_validation
	} // /function_exists check

	add_action( WM_METABOX_HOOK_PREFIX . 'saving_' . 'image', 'wm_field_image_validation', 10, 3 );
 ...
 */
	 ...
	 */
	if ( ! function_exists( 'wm_field_gallery' ) ) {
		function wm_field_gallery( $field, $page_template = null ) {
			//Field definition array
				$field = wp_parse_args( (array) $field, array(
						//DEFAULTS:
						//* = Required setting
	...
	add_action( WM_METABOX_HOOK_PREFIX . 'render_' . 'gallery', 'wm_field_gallery', 10, 2 );
	 ...
	 * @version     1.0
	 */
	if ( ! function_exists( 'wm_field_gallery_validation' ) ) {
		function wm_field_gallery_validation( $new, $field, $post_id ) {
			$new = preg_replace( '/[^0-9\,]/', '', $new );

			return $new;
		} // /wm_field_gallery_validation
	} // /function_exists check

	add_action( WM_METABOX_HOOK_PREFIX . 'saving_' . 'gallery', 'wm_field_gallery_validation', 10, 3 );
	 ...
	 * @version     1.0
	 */
	if ( ! function_exists( 'wm_field_gallery_AJAX' ) ) {
		function wm_field_gallery_AJAX() {
			...
			//Output
				echo $output;
				die();
		} // /wm_field_gallery_AJAX
	} // /function_exists check

	add_action( 'wp_ajax_wm-gallery-preview-refresh', 'wm_field_gallery_AJAX' );
