 ...
 */
	 ...
	 * @version     1.0
	 */
	if ( ! function_exists( 'wm_field_repeater' ) ) {
		function wm_field_repeater( $field, $page_template = null ) {
			...
			//Value processing
				$values = (array) wm_meta_option( $field['id'] );
						...
						if ( is_array( $values ) && ! empty( $values ) ) {
							...
							foreach ( $values as $value ) {
								if ( is_array( $field['fields'] ) && ! empty( $field['fields'] ) ) {
									$output = "\r\n\t\t" . '<table class="repeater-cell"><thead>';

										//Controls row
											$output .= "\r\n\t\t\t" . '<tr class="repeater-cell-controls"><th colspan="2">';
											$output .= "\r\n\t\t\t\t" . '<a class="button-move-cell" data-id="' . $field['id'] . '" title="' . __( 'Move the item', 'wm_domain' ) . '">#</a>';
	...
	add_action( WM_METABOX_HOOK_PREFIX . 'render_' . 'repeater', 'wm_field_repeater', 10, 2 );
