...
jQuery( function() {
	 ...
	 */
		...
		if ( jQuery().wpColorPicker ) {
			...
			jQuery( '.wm-meta-wrap .color-wrap .fieldtype-text' ).wpColorPicker( wmColorPickerOptionsArray );
		}
	 ...
	 */
		...
		jQuery( '.button-default-value' ).on( 'click', function() {
			var $this        = jQuery( this ),
			    defaultInput = $this.data( 'option' ),
			    defaultValue = $this.find( 'span' ).text();

			if ( $this.hasClass( 'default-gallery' ) ) {
				jQuery( '#' + defaultInput ).attr( 'value', defaultValue );
				jQuery( '.gallery-' + defaultInput ).html( '' );
			} else if ( $this.hasClass( 'default-slider' ) && jQuery().slider ) {
				jQuery( '#' + defaultInput + '-slider' ).slider( 'option', 'value', defaultValue );
				jQuery( '#' + defaultInput ).attr( 'value', defaultValue );
			} else {
				var defaultField = '.wm-meta-wrap [name="' + defaultInput + '"]';

				if ( 'radio' === jQuery( defaultField ).attr( 'type' ) ) {
					jQuery( defaultField + '[value="' + defaultValue + '"]' ).attr( 'checked', 'checked' );
				} else {
					jQuery( defaultField ).val( defaultValue ).change();
				}
			}
		} );
	 ...
	 */
		...
		//Upload multiple images (gallery) action
			jQuery( '.wm-meta-wrap .gallery-wrap' ).on( 'click', '.button-set-gallery', function( e ) {
				...
				frame.state( 'gallery-edit' ).on( 'update', function( selection ) {
					...
					//Remove existing images and display loader animation
						jQuery( '.gallery-' + galleryID ).find( 'a' ).hide();
						jQuery( '.gallery-loading-' + galleryID ).show();
					...
					//Do AJAX and get response (result)
						if ( imageIDarray.length ) {
							jQuery.post( ajaxurl, {
								...
								//Load new images
									jQuery( '.gallery-' + galleryID ).html( response );
								//Hide loader animation
									jQuery( '.gallery-loading-' + galleryID ).hide();
							} );
						}
	 ...
	 */
		...
		//Upload image action
			jQuery( '.wm-meta-wrap .image-wrap label, .wm-meta-wrap .button-set-image' ).on( 'click', function( e ) {
				...
				wp.media.editor.send.attachment = function( props, attachment ) {
					//Set input field values
						jQuery( 'input[name="' + uploadFieldId + '[url]"]' ).val( attachment.url );
						jQuery( 'input[name="' + uploadFieldId + '[url]"]' ).attr( 'data-preview', attachment.sizes.thumbnail.url );
						jQuery( 'input[name="' + uploadFieldId + '[id]"]' ).val( attachment.id );

					//Set preview image
						jQuery( 'div.image-' + uploadFieldId ).removeClass( 'hide' ).find( 'img' ).attr( 'src', attachment.sizes.thumbnail.url );
		...
		//Live image preview change
			jQuery( '.wm-meta-wrap .preview-enabled .fieldtype-image' ).on( 'change', function() {
				var $this         = jQuery( this ),
				    imageSrc      = $this.val(),
				...
				if ( imageSrc ) {
				  imageSrc = ( $this.data( 'preview' ) ) ? ( $this.data( 'preview' ) ) : ( $this.val() ),
					jQuery( 'div.image-' + uploadFieldId ).find( 'img' ).attr( 'src', imageSrc );
				} else {
					$this.data( 'preview', '' );
					jQuery( 'div.image-' + uploadFieldId ).find( 'img' ).attr( 'src', noThumbSrc );
				}
			} );

			//Trigger action on page load
				jQuery( '.wm-meta-wrap .fieldtype-image' ).change();
	 ...
	 */
		...
		jQuery( '.wm-meta-wrap .confirm' ).on( 'click', function( e ) {
			var btnUrl   = jQuery( this ).attr( 'href' ),
			    modalBox = jQuery( '.wm-meta-wrap .modal-box' ).fadeIn();

			modalBox.find( 'a' ).on( 'click', function() {
	 ...
	 */
		...
		if ( jQuery().slider ) {
			jQuery( '.wm-meta-wrap .slider-wrap' ).each( function() {
				...
				//Make the input field read-only
				 jQuery( sliderOptionId ).attr( 'readonly', 'readonly' );

				//Initialize jQuery UI Slider
					jQuery( sliderOptionId + '-slider' ).slider( {
						value : sliderOptionVal,
						min   : sliderOptionMin,
						max   : sliderOptionMax,
						step  : sliderOptionStep,
						slide : function( event, ui ) {
							jQuery( sliderOptionId ).val( ui.value );
						}
					} );
			} );
		}
	 ...
	 */
		...
		//Add "active" class for radio button items if custom label is used
			jQuery( '.wm-meta-wrap .radio-wrap.custom-label input:checked' ).parent( '.inline-radio' ).addClass( 'active' );
			jQuery( '.wm-meta-wrap .radio-wrap.custom-label input' ).on( 'change', function() {
				jQuery( this ).parent( '.inline-radio' ).addClass( 'active' ).siblings( '.inline-radio' ).removeClass( 'active' );
			} );
	 ...
	 */
		...
		//Adding new cells (items)
			jQuery( '.wm-meta-wrap .button-add-cell' ).on( 'click', function( e ) {
				e.preventDefault();
				...
				//Reactivate remove button when more than one item in stack
					if ( cellsWrapper.children().length > 1 ) {
						cellsWrapper.find( '.button-remove-cell' ).removeClass( 'button-disabled' );
					}
			} );

		//Removing existing cells (items)
			jQuery( '.wm-meta-wrap .button-remove-cell' ).on( 'click', function( e ) {
				var $this        = jQuery( this ),
				...
				//Remove cell only if there is more than one in the stack
					if ( $this.closest( '.repeater-cells' ).children().length > 1 ) {
						$this.closest( '.repeater-cell' ).remove();
					}

				//Deactivate remove button when only one item in stack
					if ( cellsWrapper.children().length <= 1 ) {
						cellsWrapper.find( '.button-remove-cell' ).addClass( 'button-disabled' );
					}
		...
		//Enable sorting of the cells (items)
			if ( jQuery().sortable ) {
				jQuery( '.wm-meta-wrap .repeater-cells' ).sortable( {
					cursor  : 'move',
	 ...
	 */
		...
		if ( jQuery().tabs ) {
			jQuery( '.wm-meta-wrap.jquery-ui-tabs' ).tabs();
		}
	 ...
	 */
		...
		//Toggle sub-sections
			jQuery( '.wm-meta-wrap .option-heading.toggle' ).not( '.open' ).closest( 'tbody' ).next( 'tbody' ).hide();
			jQuery( '.wm-meta-wrap .option-heading.toggle' ).on( 'click', function() {
				jQuery( this ).closest( 'tbody' ).next( 'tbody' ).toggle();
			} );
		...
		//Function to control display CSS class upon Visual Composer visibility
			function wm_vc_visible() {
				if ( jQuery( '#wpb_visual_composer' ).is( ':visible' ) ) {
					jQuery( 'body' ).addClass( 'wm-visual-composer-on' );
				} else {
					jQuery( 'body' ).removeClass( 'wm-visual-composer-on' );
				}
			} // /wm_vc_visible

		//Cache Visual Composer switch button
			var vcButton = jQuery( '.composer-switch a' );
	 ...
	 */
		...
		//Upload ZIP file action
			jQuery( '.wm-meta-wrap .zip-wrap label, .wm-meta-wrap .zip-wrap .fieldtype-zip, .wm-meta-wrap .button-set-zip' ).on( 'click', function( e ) {
				//Check if the wp.media.editor API exists.
					if ( typeof wp === 'undefined' || ! wp.media || ! wp.media.editor ) {
						return;
					}
				...
				wp.media.editor.send.attachment = function( props, attachment ) {
					...
					if ( 1 < attachment.url.search( '.zip' ) || 1 < attachment.url.search( '.ZIP' ) ) {
						...
						jQuery( 'input[name="' + uploadFieldId + '[url]"]' ).val( '' );
						jQuery( 'input[name="' + uploadFieldId + '[id]"]' ).val( '' );
					}
	 ...
	 */
		...
		//Required select field changes on load
			jQuery( 'select[name="page_template"], .wm-meta-wrap select' ).change();
